"""
Entry point cho DCRNN baseline.
Usage:
  python -m baselines.dcrnn.run --dataset PEMS04 --epochs 50
  python -m baselines.dcrnn.run --dataset PEMS04 --smoke-test
  python -m baselines.dcrnn.run --dataset PEMS08 --seed 42 --fold 0
"""
...
from baselines.dcrnn.model import DCRNN
...
def get_args():
    parser = argparse.ArgumentParser(description="DCRNN Baseline")
    ...
    parser.add_argument("--K", type=int, default=2, help="Diffusion steps (K-hop)")
...
def main():
    ...
    ckpt_tag = f"dcrnn_{args.dataset.lower()}_seed{args.seed}_{fold_tag}"
    ...
    results = train(
        ...
        device=device,
        model_name=f"DCRNN_{args.dataset}",
        save_dir=save_dir,
    ...
    out_path = os.path.join(results_dir, f"dcrnn_{args.dataset.lower()}_seed{args.seed}_{fold_tag}.json")
    out_data = {
        "model": "DCRNN",
